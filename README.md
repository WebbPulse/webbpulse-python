...
| `webbpulse.dynamodb` | `Repository` with `set_attributes` and `get_many`, `Page`, `table_name`, `ttl_at`, `ttl_in`, `encode_numbers`, `new_ulid`, `IdempotencyStore`, and the `DynamoError` family, whose `ConditionFailed` every conditional write raises | [data-access.md](docs/data-access.md) |
| `webbpulse.storage` | `presigned_put`, `PresignedUpload`: a presigned S3 PUT bounded by a signed content type and content length; `presigned_get`, `PresignedDownload`: a presigned S3 GET with optional signed response headers | [data-access.md](docs/data-access.md) |
