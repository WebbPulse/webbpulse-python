...
The port resolution is the part worth covering closely. Binding a port the Web Adapter is
not polling is the most common Web Adapter misconfiguration and it presents as the readiness
check never passing, with no application logs at all, so a regression here is expensive to
diagnose in production and cheap to catch in a unit test.
"""
...
def test_a_non_numeric_port_is_ignored_and_warned(
    ...
    """A bad value must not crash the entrypoint before it can serve anything.

    Falling back to the default keeps the container answering the readiness check, which is
    a far better failure than a process that exits during init.
    """
...
def test_run_uvicorn_binds_the_resolved_port(monkeypatch: MonkeyPatch) -> None:
    ...
    # A container must bind every interface: the adapter does not reach it over loopback.
    assert captured["host"] == "0.0.0.0"
    # log_config=None leaves the JSON formatter from webbpulse.logging in place instead of
    # letting uvicorn install its own handlers with propagate=False.
    assert captured["log_config"] is None
    # The access log duplicates what API Gateway already records, at CloudWatch cost.
...
def test_the_module_docstring_documents_the_pinned_adapter_image() -> None:
    ...
    # /opt/extensions is the only directory Lambda starts extension binaries from.
